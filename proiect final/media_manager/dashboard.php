...
// verificam daca ne aflam intr-un folder
$current_folder_id = isset($_GET['folder_id']) ? $_GET['folder_id'] : null;
if ($current_folder_id) {
    $stmtCheck = $pdo->prepare("SELECT id, name FROM folders WHERE id = :id AND user_id = :user_id");
    $stmtCheck->execute(['id' => $current_folder_id, 'user_id' => $user_id]);
    $folder_rez = $stmtCheck->fetch();
    if (!$folder_rez) {
        $current_folder_id = null; 
    }
}

// calcul widget spatiu de stocare
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['imagine'])) {
    ...
    $upload_folder_id = !empty($_POST['folder_id']) ? $_POST['folder_id'] : null;
        ...
        if (in_array($file_type, $allowed_types)) {
            ...
            if (($total_used + $trash_used + $simulated_size) <= $max_storage) {
                ...
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    $stmtInsert = $pdo->prepare("INSERT INTO images (user_id, folder_id, filename, title, size_mb) VALUES (:user_id, :folder_id, :filename, :title, :size)");
                    $stmtInsert->execute([
                        'user_id' => $user_id,
                    ...
                    $redirect_url = $upload_folder_id ? "dashboard.php?folder_id=" . $upload_folder_id : "dashboard.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'delete') {
        if ($current_folder_id) {
            $stmtDelete = $pdo->prepare("UPDATE images SET folder_id = NULL WHERE id = :id AND user_id = :user_id AND folder_id = :folder_id");
            $stmtDelete->execute(['id' => $image_id, 'user_id' => $user_id, 'folder_id' => $current_folder_id]);
        } else {
    ...
    $redirect_url = $current_folder_id ? "dashboard.php?folder_id=" . $current_folder_id : "dashboard.php";
    header("Location: " . $redirect_url); exit;
}

// preluarea folderelor utilizatorului 
...
// preluam imaginile
$sqlImages = "SELECT * FROM images WHERE user_id = :user_id AND is_deleted = 0";
$params = ['user_id' => $user_id];
if ($current_folder_id) {
    $sqlImages .= " AND folder_id = :folder_id";
    $params['folder_id'] = $current_folder_id;
}
$sqlImages .= " ORDER BY upload_date DESC";
